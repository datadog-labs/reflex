...
export const helpDetailsHtml = "<p>Increase traffic or inject slowdowns and errors. Opening a circuit blocks new requests so outstanding work can drain.</p>\n<p>Each circuit has three states:</p>\n<ul>\n<li><strong>Closed:</strong> Requests flow normally.</li>\n<li><strong>Open:</strong> New requests are blocked.</li>\n<li><strong>Probe:</strong> Requests test recovery one at a time. Five consecutive successes close the circuit; any error or timeout reopens it.</li>\n</ul>\n<p>Jev uses recent request outcomes to recommend an action: <strong>open, probe, or no change</strong>. Reflex checks the rules before applying it: opening requires 10 recent responses (within five simulated seconds in local mode, or a complete Datadog window in telemetry mode); probing requires a three-second cooldown and allows one request at a time. Stale recommendations and responses from earlier circuit generations cannot change the state.</p>";
