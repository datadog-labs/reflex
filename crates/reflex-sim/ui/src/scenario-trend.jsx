...
export function ScenarioTrend({scenario,state,selected}){
  ...
  if(scenario==='recovery')series=[{name:'Essential success (%)',points:state.history.map(p=>[p.at_ms,p.essential_success*100]),color:'purple'},{name:'Ready replicas',points:state.history.map(p=>[p.at_ms,p.ready]),color:'orange',right:true}];
  ...
  return <TimeChart series={series} start={start} end={end} leftLabel={scenario==='capacity'?'CPU':scenario==='recovery'?'Success (%)':'Jobs'} leftInteger={scenario==='scheduler'} rightInteger leftMax={scenario==='recovery'?100:undefined} rightLabel={scenario==='recovery'?'Ready replicas':undefined} rightMax={4} label={`${scenario} simulation trends`}/>;
}
