...
export function Topology({state,focus,onFocus,panelOpen,setPanelOpen}) {
  const nodes=[],edges=[];
  state.services.forEach((s,i)=>{
    ...
    nodes.push({key:`service-${i}`,column:2,kind:'service',name:s.definition.name,selected:focus===i,phase:phaseName(s.state.phase),status:level(s.state.phase),metrics:[`${fmt(s.state.queued)} queued`,`${fmt(s.state.stress*100)}% stress`,`${fmt(s.state.offered_rate)} req/s`],ratio:s.state.stress,metricLevels:['default',s.state.stress>.65?'danger':s.state.stress>.3?'warning':'default','default'],annotation:Object.values(s.faults).some(Boolean)?'Fault injected':undefined});
  ...
  return <div className="topology-canvas" aria-label="Interactive request topology">
    ...
    <div className="canvas-caption"><Text weight="bold">Circuit breaker topology</Text><Text size="sm" variant="secondary">Clients → gateway → downstream services</Text></div>
