...
export function Topology({state,focus,onFocus,panelOpen,setPanelOpen,playback}) {
  ...
  state.services.forEach((s,i)=>{
    ...
    nodes.push({key:`service-${i}`,column:2,kind:'service',name:s.definition.name,selected:focus===i,metrics:[`${fmt(s.state.queued)} queued`,`${fmt(s.state.stress*100)}% utilization`,`${fmt(s.state.offered_rate)} req/s`],ratio:s.state.stress,metricLevels:['default',s.state.stress>.65?'danger':s.state.stress>.3?'warning':'default','default'],annotation:Object.values(s.faults).some(Boolean)?'Fault injected':undefined});
  ...
  nodes.push({key:'gateway',column:1,kind:'hub',name:'gateway',subtext:'Protected by Reflex',selected:focus===null,metrics:[`${fmt(state.services.reduce((n,s)=>n+s.state.offered_rate,0))} req/s`],circuits:state.services.map(s=>({name:s.definition.name,phase:phaseName(s.state.phase),status:level(s.state.phase)}))});
