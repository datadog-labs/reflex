...
ROOT=pathlib.Path(__file__).resolve().parents[2];out=ROOT/'output/capacity-study-efficiency'
def replay(seed):
 log=out/f'replay-{seed}.log'
 if log.exists() and log.read_text().count('REPLAY VERIFIED')==12:return
 cmd=[str(ROOT/'target/release/examples/capacity_study'),'--output',str(out),'--seeds','1','--first-seed',str(seed),'--forecasts','cache','--policies','jev,jev_toto','--prompt-file',str(ROOT/'studies/capacity/prompts/efficiency-v1.txt'),'--tuning',str(ROOT/'output/capacity-study-tuning/selected.json'),'--replay']
